...
angular.
  module('candidato').
  component('candidatoUpdateQuestionarioDeHabilidades', {
     templateUrl: '/candidato/candidato-update/candidato-update-questionario-de-habilidades.template.html',
     controller: ['$rootScope', '$scope', 'DataValidation', '$location', '$window',
       function CandidatoUpdateQuestionarioDeHabilidadesController($rootScope, $scope, DataValidation, $location, $window) {
         $scope.questionarioDeHabilidades =  $rootScope.updateQuestionarioDeHabilidades != null
           ? $rootScope.updateQuestionarioDeHabilidades
           : {};
         ...
         $scope.next = function () {
           ...
           if (isValid) {
             $rootScope.updateQuestionarioDeHabilidades = $scope.questionarioDeHabilidades;
             $location.path("/candidato/updateinformacoesparapagamento");
           } else {
             $window.scrollTo(0, 0);
           }
         }
